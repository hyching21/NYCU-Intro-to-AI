...
class Ngram:
    ...
    def get_ngram(self, corpus_tokenize: List[List[str]]):
        ...
        for document in corpus_tokenize:
            for i in range(len(document)-1):
                word1, word2 = document[i], document[i+1]
                model[word1][word2] += 1
    ...
    def train_sentiment(self, df_train, df_test):
        ...
        train_corpus_embedding = np.zeros(shape = (len(df_train['review']), feature_num)) # (row, column)
        test_corpus_embedding = np.zeros(shape = (len(df_test['review']), feature_num))
        #train
        for i in tqdm(range(len(df_train['review'])), desc = 'Training_data'):
            tokens = self.tokenizer.tokenize(df_train['review'][i])
            feature_counts = Counter([(tokens[j], tokens[j+1]) for j in range(len(tokens)-1)])  # Count bi-grams
            for k , (feature, count) in enumerate(features):
                train_corpus_embedding[i, k] = feature_counts[feature]
        #test
        for i in tqdm(range(len(df_test['review'])), desc = 'Testing_data'):
            tokens = self.tokenizer.tokenize(df_test['review'][i])
            feature_counts = Counter([(tokens[j], tokens[j+1]) for j in range(len(tokens)-1)])  # Count bi-grams
            for k , (feature, count) in enumerate(features):
                test_corpus_embedding[i, k] = feature_counts[feature]
        # end your code
