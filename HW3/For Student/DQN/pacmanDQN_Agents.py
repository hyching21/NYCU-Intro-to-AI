...
class PacmanDQN(PacmanUtils):
    def __init__(self, args):        
		...
		# init model
        if(model_trained == True):
            self.policy_net = torch.load('pacman_policy_net.pt', map_location=torch.device('cpu')).to(self.device)
            self.target_net = torch.load('pacman_target_net.pt', map_location=torch.device('cpu')).to(self.device)
        else:
